...
export default function PrescribeModal({ patient, patientProfile, doctor, onClose, onSuccess }) {
  const emptyMed = () => ({
    ...
    frequency: "", instructions: "", commonDosages: [],
  });
  ...
  function addRow() { setMeds(m => [...m, emptyMed()]); }
  ...
  async function submit(e) {
    ...
    if (!valid.length) { setErr("Please add at least one medication."); return; }
    setBusy(true); setErr("");
    try {
      const pharmacistId = patientProfile?.primary_pharmacist_id || null;
      const { data: rx, error: rxErr } = await supabase
        .from("prescriptions")
        .insert({ patient_id: patient.id, doctor_id: doctor.id, pharmacist_id: pharmacistId, status: "pending_pharmacist", notes: notes.trim() || null })
        .select("id").single();
    ...
    <motion.div className="ov" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          <button onClick={onClose} type="button" style={{ width: 30, height: 30, borderRadius: 9, border: "1px solid var(--b1)", background: "var(--s2)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: t3 }}>
            <X size={13} />
          </button>
        </div>

        {/* No pharmacist warning */}
        {!patientProfile?.primary_pharmacist_id && (
          <p style={{ color: "var(--am)", fontSize: 12, marginBottom: 12, padding: "8px 12px", background: "rgba(245,158,11,.1)", borderRadius: 8 }}>
        ...
        <form onSubmit={submit}>
          {meds.map((m, i) => (
            <div key={i} style={{ marginBottom: 14, padding: 12, background: "var(--s2)", borderRadius: 12, border: "1px solid var(--b0)" }}>
              ...
              {/* Dosage: amount + unit + quick-pick chips */}
              ...
              {/* Instructions */}
              <label style={{ color: t3, fontSize: 11, fontWeight: 500, marginBottom: 4, display: "block" }}>Instructions (optional)</label>
              <input className="inp" placeholder="e.g. Take with food, avoid alcohol..."
          ...
          <button type="button" onClick={addRow} style={{
            ...
            color: t3, fontSize: 12, fontWeight: 600, cursor: "pointer",
          ...
          <div style={{ display: "flex", gap: 9 }}>
            ...
            <button type="submit" className="btn-doc" style={{ flex: 1 }} disabled={busy}>
              {busy ? <Loader2 size={14} className="auth-spin" /> : "Send to pharmacist"}
            </button>
          </div>
        </form>
      </motion.div>
