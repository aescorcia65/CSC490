...
export default function Dashboard({ user, meds, setMeds, onAdd, onEdit, onDelete, onChat, displayName, onEditName }) {
  ...
  const toggle = useCallback(async (id) => {
    ...
    if (wasTaken) await unlogMedicationTaken(user.id, id);
    else await logMedicationTaken(user.id, id);
    ...
    setModal({
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {meds.length === 0 ? <p style={{ color: t3, fontSize: 13, textAlign: "center", padding: "20px 0" }}>No medications yet.</p> :
        ...
        {drugInfo && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setDrugInfo(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 90, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <motion.div initial={{ y: 24, opacity: 0, scale: .97 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 16, opacity: 0 }} transition={{ type: "spring", damping: 26, stiffness: 300 }} onClick={e => e.stopPropagation()} style={{ background: "var(--bg)", borderRadius: 20, width: "100%", maxWidth: 560, maxHeight: "88vh", display: "flex", flexDirection: "column", border: "1px solid var(--b1)", boxShadow: "0 24px 64px rgba(0,0,0,.22)" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--b1)", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                <div style={{ width: 38, height: 38, borderRadius: 11, background: "var(--pd)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Info size={16} color="var(--p)" /></div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h3 style={{ color: t1, fontSize: 16, fontWeight: 700, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{drugInfo.med.name}</h3>
                  <p style={{ color: t3, fontSize: 12, margin: 0 }}>{drugInfo.med.dosage} · Drug Information</p>
                </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
                {drugInfo.loading && (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "40px 0", color: t3 }}>
                    ...
                    <span style={{ fontSize: 13 }}>Looking up drug information…</span>
                  </div>
                )}
                {!drugInfo.loading && !drugInfo.data && (
                  <div style={{ textAlign: "center", padding: "32px 0" }}>
                    ...
                    <p style={{ color: t3, fontSize: 13 }}>Drug information for "{drugInfo.med.name}" is not available in our database. Please consult your pharmacist or prescribing doctor.</p>
                  </div>
                )}
                {!drugInfo.loading && drugInfo.data && (() => {
                  ...
                  const sections = [
                    ...
                    { icon: Info, color: t2, bg: "var(--s2)", border: "var(--b1)", title: "Drug Interactions", content: d.drug_interactions?.[0] },
                    { icon: Info, color: t2, bg: "var(--s2)", border: "var(--b1)", title: "Pregnancy & Nursing", content: d.pregnancy?.[0] || d.nursing_mothers?.[0] },
                  ].filter(s => s.content);
                  return sections.map(({ icon: Icon, color, bg, border, title, content }) => (
                    <div key={title} style={{ borderRadius: 14, border: `1px solid ${border}`, background: bg, overflow: "hidden" }}>
                      <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ color, fontSize: 12.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em" }}>{title}</span>
                      </div>
                      <div style={{ padding: "0 16px 14px" }}>
                        <p style={{ color: t1, fontSize: 13.5, lineHeight: 1.8, margin: 0, whiteSpace: "pre-wrap" }}>
                ...
                <p style={{ color: t3, fontSize: 11, textAlign: "center", marginTop: 4 }}>Source: FDA Drug Label Database · Always follow your doctor's instructions.</p>
              </div>
            </motion.div>
          </motion.div>
        )}
