...
export async function addMedication({ name, dosage, freq, time, times, color, userId }) {
  if (!userId) return;
  try {
...
export async function updateMedication(medicationId, { name, dosage, freq, time, times, color }) {
  try {
    ...
    console.error("Supabase updateMedication error:", e);
    throw e;
  }
}
